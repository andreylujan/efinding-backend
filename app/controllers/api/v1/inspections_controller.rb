...
class Api::V1::InspectionsController < Api::V1::JsonApiController
  ...
  def transition
    ...
    if transition_name == "sign"
      if @inspection.state == "first_signature_pending"
        @inspection.initial_signer = current_user
      elsif @inspection.state == "final_signature_pending"
        @inspection.final_signer = current_user
      end
    end
